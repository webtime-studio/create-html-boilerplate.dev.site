...
sidebar_label: "Структура каталогов"
slug: /folder-structure
---

## Обзор содержимого проекта
...
так как они будут сгенерированы автоматически во время установки проекта. Структура
файлов в документации может слегка отличаться от структуры файлов в boilerplate,
потому что документация обычно обновляется немного позже основного проекта.
```

## .github

Здесь находятся шаблоны для создания issue на GitHub. `bug_report.md` если вы нашли баг
в проекте и `feature_request.md` если есть идея для улучшения или вы хотите запросить
новую фичу.

## source

Это основная папка проекта – здесь происходит всё действие. Все шаблоны, материалы, стили
и прочие файлы находится в этой директории и в её поддиректориях.

### fonts

Сюда следует помещать шрифты, которые вы используете в проекте.

### html

Здесь находятся шаблоны страниц и включаемых блоков
...
Файлы, которые будут находиться в корне проекта.
...
Конфигурация webpack и всё, что к ней относиться.
